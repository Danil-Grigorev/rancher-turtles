...
.PHONY: release-chart
release-chart: chart-generate verify-release-chart-generate $(HELM) $(KUSTOMIZE) $(RELEASE_DIR) $(CHART_RELEASE_DIR) $(CHART_PACKAGE_DIR) $(NOTES) ## Builds the chart to publish with a release
	...
	$(NOTES) --repository $(REPO) -workers=1 -add-kubernetes-version-support=false > $(CHART_RELEASE_DIR)/RELEASE_NOTES.md
